...
export default function PortfolioPage() {
  ...
  const { 
    ...
    loading: evmLoading, 
    refetch: refetchEvm 
  } = useEvmBalances(privyUserId || '');

  const [activeTab, setActiveTab] = useState<'overview' | 'wallets' | 'transfer'>('overview');
  ...
  const mainUsdc = evmBalances.find(b => b.asset === 'USDC')?.balance || 0;

  return (
    <div className="min-h-screen bg-[#0b0e11] text-white">
      {/* Top Header Section */}
      <div className="border-b border-[#2b3139] bg-[#161a1e] px-6 py-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-2xl font-bold mb-1">Portfolio Account</h1>
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-[#848e9c] text-xs uppercase tracking-wider mb-1">Total Net Worth</p>
              <p className="text-3xl font-bold text-[#fcd535]">{formatUSD(hlValue + (evmBalances.reduce((acc, b) => acc + b.usdValue, 0)))}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-6">
        <div className="flex border-b border-[#2b3139] gap-8">
          {(['overview', 'wallets', 'transfer'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`pb-4 text-sm font-medium transition-colors relative ${
                activeTab === tab ? 'text-[#fcd535]' : 'text-[#848e9c] hover:text-white'
              }`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {activeTab === 'overview' && (
              <>
                {/* Hyperliquid Summary */}
                <Card className="bg-[#161a1e] border-[#2b3139] overflow-hidden">
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-lg">Hyperliquid Trading Account</h3>
                          <p className="text-xs text-[#848e9c]">Isolated margin trading on Hyperliquid L1</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                        <p className="text-[#848e9c] text-xs mb-1 uppercase tracking-wider">Account Value</p>
                        <p className="text-xl font-bold">{formatUSD(hlValue)}</p>
                      </div>
                      <div className="p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                        <p className="text-[#848e9c] text-xs mb-1 uppercase tracking-wider">Available USDC</p>
                        <p className="text-xl font-bold text-[#0ecb81]">{formatUSD(hlUsdc.available)}</p>
                      </div>
                      <div className="p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                        <p className="text-[#848e9c] text-xs mb-1 uppercase tracking-wider">In Orders</p>
                ...
                <div className="bg-[#161a1e] border border-[#2b3139] rounded-2xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-[#2b3139] flex justify-between items-center">
                    <h3 className="font-bold">Funding Account (Main Wallet)</h3>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="text-left text-[#848e9c] text-xs uppercase tracking-wider border-b border-[#2b3139]">
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-full bg-[#0b0e11] flex items-center justify-center">
                                    <Icon icon={`cryptocurrency-color:${asset.asset.toLowerCase()}`} />
                                  </div>
                                  <span className="font-medium">{asset.asset}</span>
                                </div>
                              </td>
                              <td className="px-6 py-4 text-[#848e9c] text-sm capitalize">{asset.chain}</td>
                              <td className="px-6 py-4 font-medium">{asset.balance.toFixed(4)}</td>
                              <td className="px-6 py-4 text-right font-medium">{formatUSD(asset.usdValue)}</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <td colSpan={4} className="px-6 py-12 text-center text-[#848e9c]">No assets found in main wallet</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold">Main Ethereum Wallet</h3>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => copyToClipboard(addresses?.ethereum || '')}>
                        <Icon icon="ph:copy" />
                      </Button>
                    </div>
                    <p className="mt-4 text-xs text-[#848e9c]">This is your primary custodian wallet for external deposits and withdrawals.</p>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold">Dedicated Trading Wallet</h3>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => copyToClipboard(tradingWallet?.address || '')}>
                        <Icon icon="ph:copy" />
                      </Button>
                    </div>
                    <p className="mt-4 text-xs text-[#848e9c]">Used exclusively for high-speed DEX interactions. Keep funds here to enable zero-gas trading on Hyperliquid.</p>
                  </CardContent>
                </Card>
              </div>
            )}
            ...
            {activeTab === 'transfer' && (
              <Card className="bg-[#161a1e] border-[#2b3139] border-t-4 border-t-[#fcd535]">
                <CardContent className="p-8">
                  <h3 className="text-xl font-bold mb-6">Internal Asset Transfer</h3>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="relative">
                      <div className="space-y-2">
                        <div className="p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                          <p className="text-[#848e9c] text-xs uppercase mb-2">From</p>
                          <div className="flex justify-between items-center">
                            <span className="font-bold">Main Account</span>
                            <span className="text-[#848e9c] text-sm">Main Ethereum Wallet</span>
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
                          <p className="text-[#848e9c] text-xs uppercase mb-2">To</p>
                          <div className="flex justify-between items-center">
                            <span className="font-bold">Trading Account</span>
                            <span className="text-[#848e9c] text-sm">Hyperliquid Wallet</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between mb-2">
                        <label className="text-sm text-[#848e9c]">Transfer Amount (USDC)</label>
                        <span className="text-xs text-[#848e9c]">Max: {mainUsdc.toFixed(2)} USDC</span>
                      </div>
                      <div className="relative">
                        <input 
                          type="number"
                          ...
                          className="w-full bg-[#0b0e11] border border-[#2b3139] rounded-xl px-4 py-4 text-xl font-bold focus:outline-none focus:border-[#fcd535] transition-colors"
                        />
                        <button 
                          onClick={() => setTransferAmount(mainUsdc.toString())}
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
               <button className="p-4 bg-[#161a1e] border border-[#2b3139] rounded-2xl hover:bg-[#2b3139]/50 transition-all flex flex-col items-center gap-2 group">
                  ...
                  <span className="text-xs font-bold text-[#848e9c]">Deposit</span>
               </button>
               <button className="p-4 bg-[#161a1e] border border-[#2b3139] rounded-2xl hover:bg-[#2b3139]/50 transition-all flex flex-col items-center gap-2 group">
                  <div className="p-3 bg-[#848e9c]/10 rounded-full group-hover:scale-110 transition-transform">
                    <Icon icon="ph:minus-bold" className="text-[#848e9c]" width={20} />
                  </div>
                  <span className="text-xs font-bold text-[#848e9c]">Withdraw</span>
               </button>
            </div>
