 ...
 * 
 * Creates a SpotDeposit record and calls the admin backend to register
 * a pending deposit. Returns the treasury address for the user to send USDT to.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { depositChain, depositAmount, depositFromAddress, depositToken = "USDT" } =
      await request.json();
    ...
    if (!depositFromAddress || typeof depositFromAddress !== "string" || depositFromAddress.length < 10) {
      return NextResponse.json({ error: "depositFromAddress is required" }, { status: 400 });
    }
    ...
    if ((!userWallet.tradingWallet?.walletId || !userWallet.tradingWallet?.address) &&
      ...
      console.log('[Spot Deposit] Auto-populated tradingWallet from ethereum wallet:', userWallet.tradingWallet.address);
    }
    ...
    // Call admin backend to create a pending deposit
    ...
    if (!adminRes.ok) {
      ...
      return NextResponse.json(
        { error: errData.message || "Failed to create deposit on admin backend" },
    ...
    // Create local SpotDeposit record
    const deposit = await SpotDeposit.create({
      ...
      depositAmount,
      depositFromAddress,
      treasuryAddress: adminData.treasuryAddress || adminData.deposit?.treasuryAddress,
    ...
    console.log(`[Spot Deposit] Created deposit ${deposit._id} for user ${clerkUserId}`);
