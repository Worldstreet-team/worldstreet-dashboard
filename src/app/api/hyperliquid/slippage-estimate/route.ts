...
export async function GET(request: NextRequest) {
  try {
    ...
    if (baseToken) {
      const entry = spotMeta.universe.find((u: any) => u.tokens[0] === baseToken.index);
      if (entry) bookCoin = entry.name;
    }

    const [l2, mids] = await Promise.all([
    ...
    const levels = side === "buy"
      ? l2.levels[1].map((l: any) => ({ px: parseFloat(l.px), sz: parseFloat(l.sz) }))
      : l2.levels[0].map((l: any) => ({ px: parseFloat(l.px), sz: parseFloat(l.sz) }));
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to estimate slippage" },
      { status: 500 }
    );
  }
}
