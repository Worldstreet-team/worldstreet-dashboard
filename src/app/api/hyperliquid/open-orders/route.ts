...
/**
 * GET /api/hyperliquid/open-orders
 * Fetches frontend open orders from Hyperliquid for the authenticated user.
 * Uses frontendOpenOrders for richer display data (orderType, tif, etc.)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Failed to fetch open orders" },
      { status: 500 }
    );
  }
}
