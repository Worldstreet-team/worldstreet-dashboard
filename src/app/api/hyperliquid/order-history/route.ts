 ...
 * Fetches the authenticated user's historical orders from Hyperliquid.
 * Returns up to 2000 orders with status: filled, open, canceled, triggered, rejected, marginCanceled.
 * Resolves spot coin names to human-readable.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch historical orders and spotMeta in parallel
    const [orders, spotMeta] = await Promise.all([
      info.historicalOrders({ user: address }),
      info.spotMeta(),
    ]);

    // Build coin name map: internal name → human-readable
    ...
    // Enrich orders with human-readable coin names
