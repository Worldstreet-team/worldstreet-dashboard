...
/**
 * POST /api/privy/add-wallet-signer
 *
 * Adds the server authorization key as an additional signer on all wallets
 * for the authenticated user. This enables server-side transactions
 * (deposit, withdraw, trade) without needing the user's JWT.
 *
 * Must be called once per user after wallet creation.
 */
    ...
    for (const chainType of chainTypes) {
      ...
      for (const wallet of wallets) {
        try {
          ...
          console.log(
            `[Add Signer] Added signer to ${chainType} wallet ${wallet.id}`
          );
