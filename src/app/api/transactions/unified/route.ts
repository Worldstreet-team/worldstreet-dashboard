...
function normalizeP2PStatus(status: string): UnifiedTransactionStatus {
  switch (status) {
    ...
    default: return "processing"; // awaiting_payment, payment_sent
  }
}
...
function normalizeTransfer(doc: Record<string, unknown>): UnifiedTransaction {
  return {
    id: String(doc._id),
    type: "transfer",
    subType: doc.type as string,    // send, receive, internal
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── Build MongoDB queries per model ────────────────────────────────
    ...
    // Determine which models to query based on type filter
    const shouldQuery = (type: UnifiedTransactionType) => !typeFilter || typeFilter === type;

    // Fetch extra to allow for client-side filtering (search/status)
    const fetchLimit = limit + 50; // Over-fetch to account for post-filters
    ...
    // ── Compute stats (always from unfiltered for the user) ────────────
