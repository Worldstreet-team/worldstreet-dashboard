...
export interface ISpotDeposit extends Document {
  ...
  depositAmount: number;        // Amount user sends (in deposit token)
  depositFromAddress?: string;  // User's external wallet address (for matching)

  // Treasury info
  treasuryAddress: string;      // Address shown to user for deposit
  treasuryChain: string;

  // Admin backend reference
  adminDepositId?: string;      // _id from admin backend's DepositRequest

  // User trading wallet (disbursement destination)
  tradingWalletAddress: string;
  tradingWalletId: string;

  // On-chain references
  depositTxHash?: string;       // User's on-chain USDT send
  disburseTxHash?: string;      // Admin treasury → trading wallet
  bridgeTxHash?: string;        // Trading wallet → HL Bridge2
  ...
  disbursedAmount?: number;     // Arb USDC received in trading wallet
  bridgedAmount?: number;       // USDC bridged to HL
  spotAmount?: number;          // USDC transferred to Spot wallet
