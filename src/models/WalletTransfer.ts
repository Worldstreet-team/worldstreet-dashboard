...
export type WalletTransferDirection =
  | "outgoing"        // Sending to external address
  | "incoming"        // Receiving from external address
  | "spot-to-main"    // Internal: spot → main wallet
  | "spot-to-futures"// Internal: spot → futures
  | "main-to-spot"    // Internal: main → spot (Hyperliquid deposit)
  | "futures-to-main" // Internal: futures → main
  | "main-to-futures"; // Internal: main → futures
...
export interface IWalletTransfer extends Document {
  ...
  token: string;       // e.g. "SOL", "ETH", "USDC", "USDT"
  amount: number;
