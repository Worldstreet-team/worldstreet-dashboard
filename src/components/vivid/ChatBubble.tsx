 ...
 * Assistant messages support basic markdown rendering.
 */
export default function ChatBubble({ message }: ChatBubbleProps) {
  ...
  return (
    ...
    >
      ...
      >
        {isUser ? (
          <Icon icon="ph:user-bold" height={16} />
        ) : (
          <Icon icon="ph:sparkle-bold" height={16} />
        )}
      </div>
      ...
      <div className={`flex flex-col max-w-[75%] ${isUser ? "items-end" : "items-start"}`}>
              ...
              {message.attachments.map((att, i) => (
                <div key={i}>
                  {att.type === "image" ? (
                    ...
                    >
                      <Icon icon="ph:file-text-duotone" height={16} />
        ...
        {/* Timestamp */}
        {!message.isStreaming && (
          <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-1 px-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {formattedTime}
          </span>
        )}
      </div>
