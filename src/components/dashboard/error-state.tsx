  ...
  if (compact) {
    return (
      <div className="flex items-center justify-center gap-2 py-6 text-xs text-muted-foreground">
        <Icon icon="solar:wi-fi-router-minimalistic-broken" className="h-3.5 w-3.5 text-red-400" />
        ...
        >
          <Icon icon="solar:refresh-linear" className={`h-3 w-3 ${retrying ? "animate-spin" : ""}`} />
          {retrying ? "…" : "Retry"}
        </button>
      </div>
    )
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-10">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10">
        <Icon icon="solar:wi-fi-router-minimalistic-broken" className="h-5 w-5 text-red-500" />
      </div>
