...
export async function getTonBalance(address: string) {
  try {
    ...
    throw new Error(error.message || 'Failed to fetch TON balance');
  }
}
...
export async function sendTonTransaction(
  ...
  try {
    // Validate the wallet exists and is a TON wallet
    const wallet = await privyClient.wallets().get(walletId);
    if (!wallet || wallet.chain_type !== "ton") {
      throw new Error("Invalid TON wallet");
    }

    console.log('[Privy TON] Sending transaction from', wallet.address, 'to', params.to);
    ...
    console.log('[Privy TON] Authorization context created with user JWT');

    // Use Privy's raw sign functionality for Tier 2 chains
    // Note: This is a placeholder - TON transactions via Privy may need specific implementation
    throw new Error("TON transactions via Privy not yet fully implemented. Please use a TON wallet directly.");

    // Future implementation would be similar to SUI:
    // const signResponse = await privyClient.wallets().rawSign(walletId, {
    //   params: {
    //     bytes: tonTransactionBytes,
    //     encoding: 'hex',
    ...
    throw new Error(error.message || 'Failed to send TON transaction');
  }
}
