...
export async function sendSuiTransaction(
  ...
  try {
    // Validate the wallet exists and is a Sui wallet
    const wallet = await privyClient.wallets().get(walletId);
    if (!wallet || wallet.chain_type !== "sui") {
      throw new Error("Invalid Sui wallet");
    }

    console.log('[Privy Sui] Sending transaction from', wallet.address, 'to', params.to);
    ...
    // Build Sui transaction using Sui SDK
    ...
    txb.setGasBudget(1000000); // 0.01 SUI gas budget

    // Build transaction bytes
    const txBytes = await txb.build({ client: suiClient });
    
    // For Sui, we need to create the intent message and use blake2b256
    ...
    console.log('[Privy Sui] Transaction bytes prepared, requesting signature');

    // Use Privy's raw sign functionality for Tier 2 chains
    // Pass authorization context as the third parameter
    const signResponse = await privyClient.wallets().rawSign(
      walletId, 
      {
        params: {
          bytes: intentHex,
          encoding: 'hex',
          hash_function: 'blake2b256'  // Sui uses blake2b256, not sha256
        },
    ...
    // For Sui, we need to convert the signature to the proper format
    const { toSerializedSignature } = await import('@mysten/sui/cryptography');
    ...
    // Convert Privy’s raw signature into Uint8Array
    const signatureBytes = Uint8Array.from(Buffer.from(signature.slice(2), 'hex'));

    // Serialize into Sui transaction signature
    const suiSignature = toSerializedSignature({
      signature: signatureBytes,
      signatureScheme: 'ED25519',
      publicKey, // ✅ This is a PublicKey instance, not raw bytes
    });
...
export async function getSuiBalance(address: string) {
  try {
    const suiRpcUrl = process.env.NEXT_PUBLIC_SUI_RPC_URL || 'https://fullnode.mainnet.sui.io:443';
    ...
    throw new Error(error.message || 'Failed to fetch SUI balance');
  }
}

/**
 * Get Sui wallet balance (legacy function for Privy wallet ID)
 */
export async function getSuiBalanceByWalletId(walletId: string) {
  const wallet = await privyClient.wallets().get(walletId);
  if (!wallet || wallet.chain_type !== "sui") {
    throw new Error("Invalid Sui wallet");
  }
  
  return getSuiBalance(wallet.address);
}