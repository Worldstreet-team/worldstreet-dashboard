 ...
 *
 * Extracted so it can be called directly from both:
 * - POST /api/spot/deposit/complete (user-triggered)
 * - GET /api/spot/deposit/status (auto-triggered when disbursed)
 *
 * This avoids the internal HTTP fetch that previously failed due to
 * missing Clerk auth cookies.
 */
 ...
 * Tracks incremental balance increase to avoid false positives when
 * the user already has an existing perps balance.
 * Returns the detected balance, or 0 if timeout.
 */
async function waitForHlCredit(
  ...
  // Accept 90% of expected to account for minor bridge fees
  const threshold = expectedAmount * 0.9;

  // Snapshot initial balance so we can detect the incremental increase
  ...
  try {
    ...
    console.log(`[CompleteDeposit] Initial HL balance: ${initialBalance} USDC`);
  } catch {
  ...
  for (let i = 0; i < HL_BALANCE_POLL_MAX_ATTEMPTS; i++) {
    try {
      ...
      console.warn(`[CompleteDeposit] HL balance poll error (attempt ${i + 1}):`, err);
    }
...
export async function completeDeposit({
  ...
  if (deposit.status === "disbursed") {
    ...
    try {
      ...
      console.log(
        `[CompleteDeposit] Bridge done: ${bridgeResult.txHash}`
      );
    } catch (bridgeErr: any) {
  ...
  if (deposit.status === "transferring") {
    try {
      // Poll HL balance until bridge deposit is credited
      const hlBalance = await waitForHlCredit(walletAddress, amount);

      if (hlBalance <= 0) {
        // HL didn't credit in time — mark partial success
