...
export async function ensureUserWallet(clerkUserId: string) {
  ...
  if (userWallet) {
    ...
    if ((!userWallet.tradingWallet?.walletId || !userWallet.tradingWallet?.address) &&
      ...
      console.log('[ensureUserWallet] Auto-populated tradingWallet from ethereum:', userWallet.tradingWallet.address);
    }
    return userWallet;
  }
  ...
  try {
    ...
    console.error("[ensureUserWallet] currentUser() failed, trying clerkClient:", e);
    try {
      ...
      console.error("[ensureUserWallet] clerkClient fallback also failed:", e2);
    }
  }
  ...
  if (userWallet) {
    ...
    await userWallet.save();
    console.log("[ensureUserWallet] Found by email, backfilled clerkUserId for:", email);
    return userWallet;
  }

  // 4. No DB record — find or create Privy user
  console.log("[ensureUserWallet] No wallet in DB, looking up Privy user for:", email);

  let privyUser: any = null;

  // 4a. Try to find existing Privy user by custom_id (Clerk user ID)
  try {
    privyUser = await privyClient.getUserByCustomId(clerkUserId);
    console.log("[ensureUserWallet] Found existing Privy user by custom_id:", privyUser.id);
  } catch {
      ...
      if (error.message?.includes("Input conflict") || error.status === 422) {
        ...
        if (existingDid) {
          try {
            ...
            console.log("[ensureUserWallet] Retrieved conflict user:", existingDid);
          } catch (e2) {
            console.error("[ensureUserWallet] Failed to get conflict user:", existingDid, e2);
            return null;
          }
        } else {
          console.error("[ensureUserWallet] Conflict but no DID in error:", error.message);
          return null;
        }
      } else {
  ...
  for (const chainType of ["ethereum", "solana"]) {
    try {
      ...
      if (walletList.length > 0) {
        ...
        console.log(`[ensureUserWallet] Found ${chainType} wallet:`, w.address);
      }
    } catch (e) {
      console.error(`[ensureUserWallet] Failed to list ${chainType} wallets:`, e);
    }
  }
