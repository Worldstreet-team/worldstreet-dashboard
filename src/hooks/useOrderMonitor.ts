...
export function useOrderMonitor(): OrderMonitorReturn {
  ...
  // Place a stop-limit order for TP or SL
  const placeStopLimit = useCallback(async (
    asset: string, side: string, size: number, triggerPrice: number, reduceOnly: boolean
  ...
  // Poll open orders and detect state changes
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const currentIds = new Set<number>((data.data || []).map((o: any) => o.oid));
